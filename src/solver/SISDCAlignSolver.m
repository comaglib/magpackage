classdef SISDCAlignSolver < handle
    % SISDCALIGNSOLVER 基于波形拐点对齐的自适应 SDC 求解器 (v1.0)
    % 
    % 核心特性:
    ...
    methods
        function obj = SISDCAlignSolver(assembler)
        ...
        function [solutionResults, info] = solve(obj, space_A, space_P, ...
            ...
            while current_time < t_max - 1e-9
                ...
                for sdc_iter = 1:obj.MaxSDCIters
                    X_old = X_slab;
                    [X_slab, max_diff, ~] = obj.performSISDCSweep(X_old, x_start, ctx);
    ...
    methods (Access = private)
        ...
        function [X_probe, found, t_split] = probeForInflection(obj, x_start, u_dot_prev, dt_try, t_start, ctx)
            ...
            % 仅迭代一次，用最小成本获取波形趋势
            [X_probe, ~, ~] = obj.performSISDCSweep(X_guess, x_start, ctx);
