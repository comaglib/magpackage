请参考run_ecore_sdc_compare.m脚本，为我构建一个同样逻辑的测试脚本run_D800KVA_compare.m，用于进行另一个D800KVA变压器模型的对比测试。D800KVA变压器的具体参数如下：
1. 区域编号为：    primary1 = [5,6,10,13];    primary2 = [14,15,19,22];    secondary1 = [7,8,11,12];    secondary2 = [16,17,20,21];    airRegion = 1;    coreRegion = [2:4,9,18,23];
2. 其中，primary1 和primary2是两个高压线圈，两个高压线圈电压相等，电流方向相反，并联连接。secondary1 和secondary2是两个低压线圈，开路。铁心区域是coreRegion 。其他区域都是airRegion 。
3. 测试脚本做开路瞬态分析。primary1的匝数为2933，电阻23.42ohm，圆形线圈。primary2和primary1的参数相同。高压线圈施加正弦电压激励，电压幅值49497 V，频率50Hz,初始相位为0.
4. 铁心材料BH曲线参数如下。
5. 仍旧是对比AcISDCSolver和TransientBDF2Solver的求解结果。TransientBDF2Solver的仿真时间段为[0,0.02]，步长5e-4。
6. 磁密对比点位置调整为[0,0,-0.6]。
7. 绘制的电流曲线为primary1和primary2的励磁电流之和。
